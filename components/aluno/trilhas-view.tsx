      ...
      import type { Aluno, Atividade, Exercicio, Trilha } from "@/lib/types"
      import { Confetti } from "@/components/confetti"
      import { ExercicioDialog } from "./exercicio-dialog"
      import { AtividadeDialog } from "./atividade-dialog"
      ...
      export function TrilhasView({ aluno }: { aluno: Aluno }) {
        ...
        // ---------- ADICIONADO: atividade lançada pela professora, selecionada para o AtividadeDialog ----------
        ...
        // ---------- "Tarefa do professor": vem de db.atividades, não de um exercício embutido ----------
        const atividadesPendentes = useMemo(() => {
          if (!trilha) return []
          return db.atividades
            .filter(
              (a) =>
                a.trilha_id === trilha.id &&
                a.turma_id === aluno.turma_id &&
                a.status === "aberta" &&
                !a.alunos_concluidos.includes(aluno.id),
            )
            .sort((a, b) => (a.criada_em < b.criada_em ? 1 : -1))
        }, [db.atividades, trilha, aluno.turma_id, aluno.id])
        ...
        // ---------- Abrir a atividade lançada pela professora ----------
        function abrirAtividade(a: Atividade) {
          if (limiteAtingido) {
            toast("Todas suas tarefas foram concluídas!", {
              description: `Próximas tarefas em ${formatarContagem(msAteProximaMeiaNoite())}.`,
              icon: <Hourglass className="size-4" />,
        ...
        // ---------- Confete quando a atividade do professor rende XP ----------
        function aoConcluirAtividade(xp: number) {
          if (xp > 0) setFire((f) => f + 1)
        }
        ...
        return (
          <div className="space-y-6 overflow-x-hidden pb-4">
            ...
            {/* Atividades reais lançadas pela professora para esta trilha/turma */}
            {atividadesPendentes.length > 0 && (
              <div className="mx-auto mt-2 w-full max-w-sm space-y-2">
                {atividadesPendentes.map((at) => (
                  <button
                    key={at.id}
                    type="button"
                    onClick={() => abrirAtividade(at)}
                    className="w-full rounded-2xl bg-muted/60 p-4 text-left ring-1 ring-border transition hover:ring-2 hover:ring-primary/40"
                  >
                    <p className="flex items-center gap-1.5 text-[11px] font-extrabold uppercase tracking-wide text-muted-foreground">
                      <ClipboardList className="size-3.5" aria-hidden="true" />
                      Tarefa do professor
                    </p>
                    <p className="mt-1 line-clamp-1 font-display text-sm font-extrabold">{at.titulo}</p>
                    {at.descricao && (
                      <p className="mt-1 line-clamp-2 text-xs text-muted-foreground">{at.descricao}</p>
                    )}
                  </button>
                ))}
              </div>
            ...
            {/* Dialog da atividade real do professor */}
            <AtividadeDialog
              atividade={atividadeSelecionada}
              alunoId={aluno.id}
              open={atividadeDialogOpen}
              onOpenChange={setAtividadeDialogOpen}
              onConcluir={aoConcluirAtividade}
            />
          </div>
        )
      }